...
def test_auth_utils():
    ...
    try:
        ...
        with patch('core.auth_utils.hash_string_to_id') as mock_hash:
            ...
            assert len(profile_ids) == 5, f"Expected 5 profile IDs, got {len(profile_ids)}"
            assert profile_ids[0] == "profile_0", f"Expected 'profile_0', got '{profile_ids[0]}'"
    ...
    for module_name in lambda_modules:
        try:
            ...
            if hasattr(module, 'lambda_handler') and callable(module.lambda_handler):
                ...
                expected_handlers = ['handle_get_profile', 'handle_upsert_profile', 'handle_delete_profile']
