...
class UserServiceDeployer(ServiceDeployer):
    ...
    def deploy(self):
        ...
        stacks = {
            "s3": {
                "name": f"vibe-dating-user-s3-{self.environment}",
                "template": "01-s3.yaml",
                "parameters": {
            ...
            "iam": {
                "name": f"vibe-dating-user-iam-{self.environment}",
                "template": "02-iam.yaml",
                "parameters": {
                    "Environment": self.environment,
                    "Region": self.parameters["ApiRegion"],
                    "MediaBucketArn": f"${{vibe-dating-user-s3-{self.environment}.MediaBucketArn}}",
                    "DynamoDBTableArn": self.core_cfg["dynamodb"]["DynamoDBTableArn"],
                    "DynamoDBKMSKeyArn": self.core_cfg["dynamodb"]["DynamoDBKMSKeyArn"],
                },
                "capabilities": ["CAPABILITY_NAMED_IAM"],
                "depends_on": ["s3"],
            },
            "lambda": {
                "name": f"vibe-dating-user-lambda-{self.environment}",
                "template": "03-lambda.yaml",
                "parameters": {
                    "Environment": self.environment,
                    "Region": self.parameters["ApiRegion"],
                    "LambdaCodeBucketName": self.core_cfg["s3"]["LambdaCodeBucketName"],
                    "LambdaExecutionRoleArn": f"${{vibe-dating-user-iam-{self.environment}.LambdaExecutionRoleArn}}",
                    "CoreLayerArn": self.core_cfg["lambda"]["CoreLayerArn"],
                    "DynamoDBTableName": self.core_cfg["dynamodb"]["DynamoDBTableName"],
                    "MediaBucketName": f"${{vibe-dating-user-s3-{self.environment}.MediaBucketName}}",
                },
                "depends_on": ["s3", "iam"],
            },
            "apigateway": {
                "name": f"vibe-dating-user-apigateway-{self.environment}",
                "template": "04-apigateway.yaml",
                "parameters": {
                    "Environment": self.environment,
                    "Region": self.parameters["ApiRegion"],
        ...
        stack_order = ["s3", "iam", "lambda", "apigateway"]
